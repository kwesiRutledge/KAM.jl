...
@testset "u_dependent_Pre() Tests" begin
...
@testset "KAM Tests" begin
    sys_5a = get_figure3_system(3)

    sys_5a_out = KAM(sys_5a,1)

    # There should be two nodes in the outputs system
    @test length(sys_5a_out.X) == 2

    sys_5a_out2 = KAM(sys_5a,2)

    @test length(sys_5a_out2.X) == 3

    sys_5a_out3 = KAM(sys_5a,3)

    println("X0")
    println(sys_5a_out3.X)

    @test length(sys_5a_out3.X) == 6
